...
struct npc_letollAI : public npc_escortAI
{
    ...
    void JustStarted()
    {
        ...
        MaNGOS::AllCreaturesOfEntryInRange check(me, NPC_RESEARCHER, 25.0f);
        MaNGOS::ObjectListSearcher<Creature, MaNGOS::AllCreaturesOfEntryInRange> searcher(ResearchersList, check);
...
bool go_veil_skith_cage(Player* player, GameObject* go)
{
    ...
    {
        MaNGOS::AllCreaturesOfEntryInRange check(player, NPC_CHILD_CAPITIVE, 5.0f);
        MaNGOS::ObjectListSearcher<Creature, MaNGOS::AllCreaturesOfEntryInRange> searcher(ChildrenList, check);
