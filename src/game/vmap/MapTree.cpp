...
bool StaticMapTree::InitMap(std::string const& fname, VMapManager2* vm)
{
    ...
    else
    {
        ...
        if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
        {
            ...
            {
                success = false;
                sLog.outBasic("StaticMapTree::InitMap() could not acquire WorldModel pointer for '%s'!", spawn.name.c_str());
            }
        }
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
{
    ...
    if (!iTreeValues)
    {
        sLog.outBasic("StaticMapTree::LoadMapTile(): Tree has not been initialized! [%u,%u]", tileX, tileY);
        return false;
    }
    bool result = true;
    ...
    if (tf)
    {
        ...
        for (uint32 i = 0; i < numSpawns && result; ++i)
        {
            ...
            if (result)
            {
                ...
                WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name);
                if (!model)
                    sLog.outBasic("StaticMapTree::LoadMapTile() could not acquire WorldModel pointer for '%s'!", spawn.name.c_str());
                ...
                if (!iLoadedSpawns.count(referencedVal))
                {
                    ...
                    {
                        sLog.outBasic("invalid tree element! (%u/%u)", referencedVal, iNTreeValues);
                        continue;
                    }
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
{
    ...
    {
        sLog.outBasic("StaticMapTree::UnloadMapTile(): Trying to unload non-loaded tile. Map:%u X:%u Y:%u", iMapID, tileX, tileY);
        return;
    }
            ...
            for (uint32 i = 0; i < numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    if (!iLoadedSpawns.count(referencedNode))
                        sLog.outBasic("Trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
