...
WorldModel* VMapManager2::acquireModelInstance(std::string const& basepath, std::string const& filename)
{
    ...
    if (model == iLoadedModelFiles.end())
    {
        ...
        {
            sLog.outBasic("VMapManager2: could not load '%s%s.vmo'!", basepath.c_str(), filename.c_str());
            delete worldmodel;
            m_modelsLock.release();
            return nullptr;
        }
...
void VMapManager2::releaseModelInstance(std::string const& filename)
{
    ...
    if (model == iLoadedModelFiles.end())
    {
        sLog.outBasic("VMapManager2: trying to unload non-loaded file '%s'!", filename.c_str());
        m_modelsLock.release();
        return;
    }
