        ...
        for (std::map<uint64, uint8>::iterator itr = m_RemovedPlayers.begin(); itr != m_RemovedPlayers.end(); ++itr)
        {
            ...
            switch (itr->second)
            {
                ...
                case 2:                                     // revive queue
                    ...
                    break;
                default:
                    sLog.outError( "ERROR: BattleGround: Unknown remove player case!");
            }
        }
...
void BattleGround::EndBattleGround(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        if (winner_arena_team && loser_arena_team)
        {
            ...
            sLog.out(LOG_CHAR,"Arena match Type: %u for Team1Id: %u - Team2Id: %u ended. WinnerTeamId: %u. Winner rating: %u, Loser rating: %u. RatingChange: %i.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE], winner_arena_team->GetId(), winner_rating, loser_rating, winner_change);
            if (sWorld.getConfig(CONFIG_ARENA_LOG_EXTENDED_INFO))
                for (BattleGroundScoreMap::const_iterator itr = GetPlayerScoresBegin();itr !=GetPlayerScoresEnd(); ++itr)
                    if (Player* player = sObjectMgr.GetPlayer(itr->first))
                        sLog.out(LOG_CHAR,"Statistics for %s (GUID: " UI64FMTD ", Team: %d, IP: %s): %u damage, %u healing, %u killing blows", player->GetName(), itr->first, player->GetArenaTeamId(m_ArenaType == 5 ? 2 : m_ArenaType == 3), player->GetSession()->GetRemoteAddress().c_str(), itr->second->DamageDone, itr->second->HealingDone, itr->second->KillingBlows);
        }
        else
        {
...
void BattleGround::Reset()
{
    ...
    if (m_InvitedAlliance > 0 || m_InvitedHorde > 0)
        sLog.outError( "ERROR: BattleGround system ERROR: bad counter, m_InvitedAlliance: %d, m_InvitedHorde: %d", m_InvitedAlliance, m_InvitedHorde);
...
void BattleGround::StartBattleGround()
{
    ...
    AnnounceBGStart();
    if (m_IsRated)
        sLog.out(LOG_CHAR,"Arena match type: %u for Team1Id: %u - Team2Id: %u started.", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE]);
}
    ...
    if (isArena())
    {
        ...
        else
        {
            ...
            if (plr->GetTemporaryUnsummonedPetNumber())
                CharacterDatabase.PExecute("DELETE FROM pet_aura WHERE guid = '%u'", plr->GetTemporaryUnsummonedPetNumber());
...
void BattleGround::UpdatePlayerScore(Player *Source, uint32 type, uint32 value)
{
    ...
    switch (type)
    {
        ...
        case SCORE_HEALING_DONE:                            // Healing Done
            ...
            break;
        default:
            sLog.outError( "ERROR: BattleGround: Unknown player score type %u", type);
            break;
    }
}
...
bool BattleGround::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    if (!go->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_GAMEOBJECT),entry, map,x,y,z,o,rotation0,rotation1,rotation2,rotation3,100,GO_STATE_READY))
    {
        ...
        sLog.outError( "ERROR: Cannot create gameobject template %u! BattleGround not created!", entry);
        delete go;
        return false;
    }
/*
...
void BattleGround::DoorClose(uint32 type)
{
    ...
    if (!tmpMap)
    {
        sLog.outError( "ERROR: BattleGround::DoorClose: map not found");
        return;
    }
    ...
    {
        sLog.outError( "ERROR: BattleGround: Door object not found (cannot close doors)");
    }
}
...
void BattleGround::DoorOpen(uint32 type)
{
    ...
    {
        sLog.outError( "ERROR: BattleGround: Door object not found! - doors will be closed.");
    }
}
...
GameObject* BattleGround::GetBGObject(uint32 type)
{
    ...
    if (!tmpMap)
    {
        sLog.outError( "ERROR: BattleGround::GetBGObject: map not found");
        return NULL;
    }

    GameObject *obj = tmpMap->GetGameObject(m_BgObjects[type]);
    if (!obj)
        sLog.outError( "ERROR: couldn't get gameobject %i",type);
    return obj;
}
...
Creature* BattleGround::GetBGCreature(uint32 type)
{
    ...
    if (!creature)
        sLog.outError( "ERROR: couldn't get creature %i",type);
    return creature;
}
...
Creature* BattleGround::AddCreature(uint32 entry, uint32 type, uint32 teamval, float x, float y, float z, float o, uint32 respawntime)
{
    ...
    {
        sLog.outError( "ERROR: Can't create creature entry: %u",entry);
        delete pCreature;
        return NULL;
    }
...
bool BattleGround::DelCreature(uint32 type)
{
    ...
    if (!cr)
    {
        sLog.outError( "ERROR: Can't find creature guid: %u",GUID_LOPART(m_BgCreatures[type]));
        return false;
    }
...
bool BattleGround::DelObject(uint32 type, bool setGoState)
{
    ...
    if (!obj)
    {
        sLog.outError( "ERROR: Can't find gobject guid: %u",GUID_LOPART(m_BgObjects[type]));
        return false;
    }
...
bool BattleGround::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    if (!pCreature)
    {
        sLog.outError( "ERROR: Can't create Spirit guide. BattleGround not created!");
        EndNow();
        return false;
    }
...
void BattleGround::AddSpectatorNPC(float x, float y, float z, float o)
{
    ...
    if (!pCreature)
        sLog.outError( "ERROR: Can't create Arena Spectator!");
...
void BattleGround::HandleTriggerBuff(uint64 const& go_guid)
{
    ...
    if (index < 0)
    {
        sLog.outError( "ERROR: BattleGround (Type: %u) has buff gameobject (Guid: %u Entry: %u Type:%u) but it hasn't that object in its internal data",GetTypeID(),GUID_LOPART(go_guid),obj->GetEntry(),obj->GetGoType());
        return;
    }
...
void BattleGround::PlayerRelogin(uint64 guid)
{
    ...
    if (!plr)
    {
        sLog.outError( "ERROR: BattleGround: Player " UI64FMTD " not found!", guid);
        return;
    }
...
int32 BattleGround::GetObjectType(uint64 guid)
{
    for (uint32 i = 0;i <= m_BgObjects.size(); i++)
        if (m_BgObjects[i] == guid)
            return i;
    sLog.outError( "ERROR: BattleGround: cheating? a player used a gameobject which isnt supposed to be a usable object!");
    return -1;
}
