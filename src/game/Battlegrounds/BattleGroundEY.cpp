    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!plr)
                {
                    sLog.outError( "ERROR: BattleGroundEY: Player " UI64FMTD " not found!", m_PlayersNearPoint[EY_POINTS_MAX][j]);
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                if (!plr)
                {
                    sLog.outError( "ERROR: BattleGroundEY: Player " UI64FMTD " not found!", m_PlayersNearPoint[i][j]);
...
bool BattleGroundEY::SetupBattleGround()
{
    ...
    for (int i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (!at)
        {
            sLog.outError( "ERROR: BattleGroundEY: Unknown trigger: %u", m_Points_Trigger[i]);
            continue;
        }
        if ( !AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REALVER + i * 3, Buff_Entries[0], at->x, at->y, at->z, 0.907571f, 0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY)
            ...
            sLog.outError( "ERROR: BattleGroundEY: Cannot spawn buff");
    }
...
void BattleGroundEY::RespawnFlagAfterDrop()
{
    ...
    if (obj)
        ...
        sLog.outError( "ERROR: BattleGroundEY: Unknown dropped flag guid: %u",GUID_LOPART(GetDroppedFlagGUID()));
...
void BattleGroundEY::EventTeamCapturedPoint(Player *Source, uint32 Point)
{
    ...
    if (!sg || !AddSpiritGuide(Point, sg->x, sg->y, sg->z, 3.124139f, Team))
        sLog.outError( "ERROR: BatteGroundEY: Failed to spawn spirit guide! point: %u, team: %u, graveyard_id: %u",
