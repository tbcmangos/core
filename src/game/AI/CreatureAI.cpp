...
AISpellEntryType * UnitAI::AISpellEntry;
 AISpellEntryType * GetAISpellEntry(uint32 i) { return &CreatureAI::AISpellEntry[i]; }
...
void CreatureAI::DoZoneInCombat(float max_dist)
{
    ...
    {
        sLog.outError( "ERROR: DoZoneInCombat call for map that isn't an instance (creature entry = %d)", creature->GetTypeId() == TYPEID_UNIT ? ((Creature*)creature)->GetEntry() : 0);
        return;
    }
