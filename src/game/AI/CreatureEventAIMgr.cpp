        ...
        do
        {
            ...
            if (temp.uiSoundId)
            {
                if (!sSoundEntriesStore.LookupEntry(temp.uiSoundId))
                    sLog.outErrorDb( "CreatureEventAI:  Entry %i in table `creature_ai_texts` has Sound %u but sound does not exist.",i,temp.uiSoundId);
            }

            if (!GetLanguageDescByID(temp.uiLanguage))
                sLog.outErrorDb( "CreatureEventAI:  Entry %i in table `creature_ai_texts` using Language %u but Language does not exist.",i,temp.uiLanguage);

            if (temp.uiType > CHAT_TYPE_ZONE_YELL)
                sLog.outErrorDb( "CreatureEventAI:  Entry %i in table `creature_ai_texts` has Type %u but this Chat Type does not exist.",i,temp.uiType);

            if (temp.uiEmote)
            {
                if (!sEmotesStore.LookupEntry(temp.uiEmote))
                    sLog.outErrorDb( "CreatureEventAI:  Entry %i in table `creature_ai_texts` has Emote %u but emote does not exist.",i,temp.uiEmote);
            }
...
void CreatureEventAIMgr::LoadCreatureEventAI_Scripts(uint32 creatureId)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (entry > 0)
            {
                ...
                {
                    sLog.outErrorDb( "CreatureEventAI:  Event %u has script for non-existing creature entry (%u), skipping.", i, entry);
                    continue;
                }
            }
            else
            {
                ...
                {
                    sLog.outErrorDb( "CreatureEventAI:  Event %u has script for non-existing creature GUID (-%u), skipping.", i, guid);
                    continue;
                }
            }
            ...
            {
                sLog.outErrorDb( "CreatureEventAI:  Creature %i are using event %u with more than 100 percent chance. Adjusting to 100 percent.", entry, i);
            ...
            switch (temp.event_type)
            {
                ...
                case EVENT_T_TIMER_OOC:
                    if (temp.timer.initialMax < temp.timer.initialMin)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i are using timed event(%u) with param2 < param1 (InitialMax < InitialMin). Event will never repeat.", entry, i);
                    if (temp.timer.repeatMax < temp.timer.repeatMin)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i are using repeatable event(%u) with param4 < param3 (RepeatMax < RepeatMin). Event will never repeat.", entry, i);
                    break;
                case EVENT_T_HP:
                case EVENT_T_MANA:
                ...
                case EVENT_T_TARGET_MANA:
                    if (temp.percent_range.percentMax > 100)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i are using percentage event(%u) with param2 (MinPercent) > 100. Event will never trigger! ", entry, i);

                    if (temp.percent_range.percentMax <= temp.percent_range.percentMin)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i are using percentage event(%u) with param1 <= param2 (MaxPercent <= MinPercent). Event will never trigger! ", entry, i);

                    if (temp.event_flags & EFLAG_REPEATABLE && !temp.percent_range.repeatMin && !temp.percent_range.repeatMax)
                    {
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i has param3 and param4=0 (RepeatMin/RepeatMax) but cannot be repeatable without timers. Removing EFLAG_REPEATABLE for event %u.", entry, i);
                ...
                case EVENT_T_SPELLHIT:
                    if (temp.spell_hit.spellId)
                    {
                        ...
                        if (!pSpell)
                        {
                            sLog.outErrorDb( "CreatureEventAI:  Creature %i has non-existant SpellID(%u) defined in event %u.", entry, temp.spell_hit.spellId, i);
                            continue;
                        }

                        if ((temp.spell_hit.schoolMask & pSpell->SchoolMask) != pSpell->SchoolMask)
                            sLog.outErrorDb( "CreatureEventAI:  Creature %i has param1(spellId %u) but param2 is not -1 and not equal to spell's school mask. Event %u can never trigger.", entry, temp.spell_hit.spellId, i);
                        ...
                        if (temp.spell_hit.schoolMask && temp.spell_hit.schoolMask != -1)
                            sLog.outErrorDb( "CreatureEventAI:  Creature %i has param1(spellId %u) but param2 is not 0. Event %u can never trigger.", entry, temp.spell_hit.spellId, i);
                    }

                    if (!temp.spell_hit.schoolMask)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u is using invalid SpellSchoolMask(%u) defined in event %u.", entry, temp.spell_hit.schoolMask, i);

                    if (temp.spell_hit.repeatMax < temp.spell_hit.repeatMin)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u are using repeatable event(%u) with param4 < param3 (RepeatMax < RepeatMin). Event will never repeat.", entry, i);
                    break;
                case EVENT_T_RANGE:
                    if (temp.range.maxDist < temp.range.minDist)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u are using event(%u) with param2 < param1 (MaxDist < MinDist). Event will never repeat.", entry, i);
                    ...
                    switch (temp.spawned.condition)
                    {
                        ...
                        case SPAWNED_EVENT_MAP:
                            if (!sMapStore.LookupEntry(temp.spawned.conditionValue1))
                                sLog.outErrorDb( "CreatureEventAI:  Creature %u are using spawned event(%u) with param1 = %u 'map specific' but with not existed map (%u) in param2. Event will never repeat.", entry, i, temp.spawned.condition, temp.spawned.conditionValue1);
                            break;
                        case SPAWNED_EVENT_ZONE:
                            if (!GetAreaEntryByAreaID(temp.spawned.conditionValue1))
                                sLog.outErrorDb( "CreatureEventAI:  Creature %u are using spawned event(%u) with param1 = %u 'area specific' but with not existed area (%u) in param2. Event will never repeat.", entry, i, temp.spawned.condition, temp.spawned.conditionValue1);
                        default:
                            sLog.outErrorDb( "CreatureEventAI:  Creature %u are using invalid spawned event %u mode (%u) in param1", entry, i, temp.spawned.condition);
                            break;
                    }
                    break;
                ...
                case EVENT_T_FRIENDLY_MISSING_BUFF:
                {
                    ...
                    {
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u has wrong radius (%u) for EVENT_T_FRIENDLY_MISSING_BUFF defined in event %u.", entry, temp.friendly_buff.radius, i);
                        continue;
                    }
                ...
                case EVENT_T_KILL:
                    if (temp.kill.repeatMax < temp.kill.repeatMin)
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u are using event(%u) with param2 < param1 (RepeatMax < RepeatMin). Event will never repeat.", entry, i);
                    break;
                ...
                case EVENT_T_SUMMONED_UNIT:
                    if (!sCreatureStorage.LookupEntry<CreatureInfo>(temp.summon_unit.creatureId))
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u are using event(%u) with not existed creature template id (%u) in param1, skipped.", entry, i, temp.summon_unit.creatureId);
                ...
                case EVENT_T_QUEST_COMPLETE:
                    if (!sObjectMgr.GetQuestTemplate(temp.quest.questId))
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u are using event(%u) with not existed qyest id (%u) in param1, skipped.", entry, i, temp.quest.questId);
                    sLog.outErrorDb( "CreatureEventAI: Creature %u using not implemented event (%u) in event %u.", entry, temp.event_id, i);
                    continue;
                ...
                case EVENT_T_REACHED_HOME:
                {
                    ...
                    {
                        sLog.outErrorDb( "CreatureEventAI:  Creature %i has EFLAG_REPEATABLE set. Event can never be repeatable. Removing flag for event %u.", entry, i);
                        temp.event_flags &= ~EFLAG_REPEATABLE;
                    }
                ...
                case EVENT_T_RECEIVE_EMOTE:
                {
                    ...
                    {
                        sLog.outErrorDb( "CreatureEventAI: Creature %u using event %u: param1 (EmoteTextId: %u) are not valid.",entry, i, temp.receive_emote.emoteId);
                        continue;
                    }
                    ...
                    {
                        sLog.outErrorDb( "CreatureEventAI: Creature %u using event %u: param2 (Condition: %u) are not valid.",entry, i, temp.receive_emote.condition);
                        continue;
                    }

                    if (!(temp.event_flags & EFLAG_REPEATABLE))
                    {
                        sLog.outErrorDb( "CreatureEventAI: Creature %u using event %u: EFLAG_REPEATABLE not set. Event must always be repeatable. Flag applied.", entry, i);
                        temp.event_flags |= EFLAG_REPEATABLE;
                    }
                ...
                case EVENT_T_TARGET_BUFFED:
                {
                    ...
                    {
                        sLog.outErrorDb( "CreatureEventAI:  Creature %u has wrong spell stack size (%u) defined in event %u.", entry, temp.buffed.amount, i);
                        continue;
                    }
                ...
                default:
                    sLog.outErrorDb( "CreatureEventAI: Creature %u using not checked at load event (%u) in event %u. Need check code update?", entry, temp.event_id, i);
                    break;
            }
    ...
    else
    {
        ...
        {
            sLog.outError( "ERROR: LoadCreatureEventAI_Scripts(uint32 creatureId = %u). Result is empty !", creatureId);
        }
    }
}
