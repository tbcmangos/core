...
void WorldSession::SendStablePet(uint64 guid)
{
    ...
    //                                                            0      1     2   3      4      5        6
    QueryResult* result = RealmDataDatabase.PQuery("SELECT owner, slot, id, entry, level, loyalty, name FROM character_pet WHERE owner = '%u' AND slot > 0 AND slot < 3",_player->GetGUIDLow());
...
void WorldSession::HandleStablePet(WorldPacket & recv_data)
{
    ...
    QueryResult* result = RealmDataDatabase.PQuery("SELECT owner,slot,id FROM character_pet WHERE owner = '%u'  AND slot > 0 AND slot < 3 ORDER BY slot ",_player->GetGUIDLow());
    if (result)
    {
        do
        {
...
void WorldSession::HandleUnstablePet(WorldPacket & recv_data)
{
    ...
    QueryResult* result = RealmDataDatabase.PQuery("SELECT entry FROM character_pet WHERE owner = '%u' AND id = '%u' AND slot > 0 AND slot < 3",_player->GetGUIDLow(),petnumber);
...
void WorldSession::HandleStableSwapPet(WorldPacket & recv_data)
{
    ...
    QueryResult* result = RealmDataDatabase.PQuery("SELECT slot,entry FROM character_pet WHERE owner = '%u' AND id = '%u'",
