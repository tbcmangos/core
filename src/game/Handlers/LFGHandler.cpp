        ...
        for (std::list<uint64>::const_iterator itr = a->second.begin(); itr != a->second.end(); ++itr)
        {
            ...
            switch (lfgType)
            {
                ...
                case 1: // LFM
                    ...
                    break;
                default:
                    sLog.outError( "ERROR: WorldSession::SendLFM: wrong lfgtype (%u) for player %u (acc %u)", lfgType, plr->GetGUIDLow(), plr->GetSession()->GetAccountId());
                    for (uint8 j = 0; j < MAX_LOOKING_FOR_GROUP_SLOT; ++j)
                        data << uint32(0x00);
                    break;
            }
...
void WorldSession::SendLfgResult(uint32 type, uint32 entry, uint8 lfg_type)
{
    switch (lfg_type)
    {
        ...
        case 1:
            ...
            break;
        default:
            sLog.outError( "ERROR: WorldSession::SendLfgResult: Wrong lfg_type (%u) for player %u (acc: %u)", lfg_type, _player ? _player->GetGUIDLow() : 0, GetAccountId());
            break;
    }
}
