    ...
    if (entry->map_type == MAP_INSTANCE || entry->map_type == MAP_RAID)
    {
        if (entry->map_type == MAP_RAID)
        {
            ...
            if (!player->IsGameMaster() && !sWorld.getConfig(CONFIG_INSTANCE_IGNORE_RAID))
            {
                ...
                if (!group || !group->isRaidGroup())
                {
                    ...
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetMangosString(810), mapName);
        ...
        if (!player->IsAlive())
        {
            if (Corpse *corpse = player->GetCorpse())
            {
                ...
                if (!instance_map)
                {
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetMangosString(811), mapName);
