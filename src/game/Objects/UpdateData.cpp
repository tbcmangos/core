...
void UpdateData::Compress(void* dst, uint32 *dst_size, void* src, int src_size)
{
    ...
    if (z_res != Z_OK)
    {
        sLog.outError( "ERROR: Can't compress update packet (zlib: deflateInit) Error code: %i (%s)",z_res,zError(z_res));
        *dst_size = 0;
        return;
    }
    ...
    if (z_res != Z_OK)
    {
        sLog.outError( "ERROR: Can't compress update packet (zlib: deflate) Error code: %i (%s)",z_res,zError(z_res));
        *dst_size = 0;
        return;
    }
    ...
    {
        sLog.outError( "ERROR: Can't compress update packet (zlib: deflate not greedy)");
        *dst_size = 0;
        return;
    }
    ...
    {
        sLog.outError( "ERROR: Can't compress update packet (zlib: deflate should report Z_STREAM_END instead %i (%s)",z_res,zError(z_res));
        *dst_size = 0;
        return;
    }
    ...
    if (z_res != Z_OK)
    {
        sLog.outError( "ERROR: Can't compress update packet (zlib: deflateEnd) Error code: %i (%s)",z_res,zError(z_res));
        *dst_size = 0;
        return;
    }
